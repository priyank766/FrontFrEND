...
def get_live_preview():
    ...
    if not ui_detection_file.exists():
        return "<h1>Live Preview Not Available</h1><p>UI detection output file not found.</p>", 204
    ...
    try:
        ...
        return "<h1>Live Preview Error</h1><p>Could not read UI detection output.</p>", 500

    example_files = ui_detection_data.get("examples", [])
    if not example_files:
        return "<h1>Live Preview Not Available</h1><p>No example UI files found in detection output.</p>", 204
    ...
    repo_root = PROJECT_ROOT.parent / "repo" # Go up one level from backend to FrontFrEND, then into repo
    ...
    if not combined_html:
        return "<h1>Live Preview Not Available</h1><p>No HTML file found in example UI files.</p>", 204

    # Embed CSS and JS into the HTML
    if css_content:
        combined_html = combined_html.replace("</head>", f"<style>{css_content}</style></head>")
    if js_content:
        combined_html = combined_html.replace("</body>", f"<script>{js_content}</script></body>")
